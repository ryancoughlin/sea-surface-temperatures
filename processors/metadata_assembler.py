...
class MetadataAssembler:
    ...
    def assemble_metadata(self, date: datetime, dataset: str, region: str, 
                         asset_paths, data: xr.DataArray = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if data is not None:
                if SOURCES[dataset]['type'] == 'currents':
                    ...
                    ranges = self._get_default_ranges_from_data(data, dataset)
            else:
                logger.warning("No data provided for metadata assembly")
                ranges = {}
